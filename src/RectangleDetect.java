...
{
	static Mat original_image = null;

	public static void main(String[] args)
	{

		System.loadLibrary(Core.NATIVE_LIBRARY_NAME);
		String datapath = "E:/EclipseProject/opencv_test/sampleimage3.jpg";
		original_image = Imgcodecs.imread(datapath, 4);
		...
		// Imgproc.threshold(binaryimage, binaryimage, 140, 255, Imgproc.THRESH_BINARY);
		...
		// corner = Mat.zeros(binaryimage.size(), CvType.CV_32F);
		Imgproc.morphologyEx(harrismat, harrismat, Imgproc.MORPH_DILATE, kernel, new Point(2, 2), 3);
		// Mat goodfetotrack = harrismat.clone();
		// Imgproc.cornerHarris(harrismat, corner, 7, 3, 0.06);
		...
		for (int i = 0; i < points.length; i++)
		{
			...
			Imgproc.putText(goodto, cornerval, points[i], Core.FONT_ITALIC, 2.5, new Scalar(255), 5);
		}

		Point[] points_4 = CalcPoints(points);

		HighGui.imshow("goodto", goodto);
		HighGui.resizeWindow("goodto", 1280, 720);
		/*
		 * Mat dstNorm = corner; Mat dstNorm2 = new Mat();
		 * 
		 * Core.normalize(corner, dstNorm, 0, 255, Core.NORM_MINMAX);
		 * Core.convertScaleAbs(dstNorm, dstNorm2);
		 * 
		 * float[] dstNormData = new float[(int) (dstNorm.total() *
		 * dstNorm.channels())]; dstNorm.get(0, 0, dstNormData); for (int i = 0; i <
		 * dstNorm.rows(); i++) { for (int j = 0; j < dstNorm.cols(); j++) { int value =
		 * (int) dstNormData[i * dstNorm.cols() + j]; if (value > 115 && value < 140) {
		 * Imgproc.circle(dstNorm2, new Point(j, i), 25, new Scalar(255), 6);
		 * System.out.println("corner : " + i + " " + j); } } }
		 */

		finalmat = ResizeMat(WarpingImage(points_4));

		HighGui.imshow("binaryimage", binaryimage);
		HighGui.resizeWindow("binaryimage", 1280, 720);
		HighGui.imshow("original", original_image);
		HighGui.resizeWindow("original", 1280, 720);
		HighGui.imshow("gray_image", gray_image);
		HighGui.resizeWindow("gray_image", 1280, 720);
		HighGui.imshow("finalmat", finalmat);
		HighGui.waitKey();
		System.exit(0);
	}

	public static Point[] CalcPoints(Point[] points) // 사이 거리가 가장 먼 두 포인트를 계산
	{
	...
	public static Mat WarpingImage(Point[] points_4)
	{
	...
	public static Mat ResizeMat(Mat mat)
	{
		...
		Imgproc.resize(mat, resized_mat, new Size(620, 877));
		return resized_mat;
	}
